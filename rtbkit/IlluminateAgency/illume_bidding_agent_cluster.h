...
#ifndef ILLUME_BIDDING_AGENT_CLUSTER_H_
#define ILLUME_BIDDING_AGENT_CLUSTER_H_
 ...
 */
class IllumeBiddingAgentCluster:
    public Datacratic::ServiceBase,
    public Datacratic::MessageLoop
{
public:
    typedef std::shared_ptr<IllumeBiddingAgent> agent_ptr;

	IllumeBiddingAgentCluster(std::shared_ptr<ServiceProxies> proxies,
                              const std::string & name = "illume_bidding_agent_cluster",
                              int numThreads = 1,
	...
	IllumeBiddingAgentCluster(ServiceBase & parent,
                              const std::string & name = "illume_bidding_agent_cluster",
                              int numThreads = 1,
                              double maxAddedLatency = 0.0005)
    ...
    virtual ~IllumeBiddingAgentCluster () { }
	...
	{
		std::vector<std::string> namesAgent;
		for (auto& agent : agents_)
			if (agent.first.get())
				namesAgent.insert(agent.first);
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<IllumeBiddingAgent>> agents_;
};

} /* namespace RTBKIT */
#endif /* ILLUME_BIDDING_AGENT_CLUSTER_H_ */
