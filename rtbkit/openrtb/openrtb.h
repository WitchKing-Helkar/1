...
/** 7.1 Native Layout IDs
    ...
    OpenRTB Native Ads API Specification Version 1.0.0.2
*/
...
/** 7.2 Native Ad Unit IDs
    ...
    An implementing exchange may not support all asset variants or introduce new ones
    unique to that system. OpenRTB Native Ads API Specification Version 1.0.0.2
    */
...
struct NativeAssetData {
    ~NativeAssetData(){}
    DataAssetTypes type;   ///< Type ID of the element supported by the publisher.
    Datacratic::TaggedInt len; ///< Maximum length of the text in the element’s response
    //ext
};
...
struct NativeAssetVideo {
    ~NativeAssetVideo(){}
    Datacratic::List<MimeType> mimes;       ///< Content MIME types supported
    Datacratic::TaggedInt minduration;  ///< Minimum video ad duration in seconds.
    Datacratic::TaggedInt maxduration;  ///< Maximum video ad duration in seconds.
    Datacratic::List<VideoBidResponseProtocol> protocols; ///< Bid response protocols array (table 6.7) (OpenRTB2.2)
    Json::Value ext;                    ///< This object is a placeholder that may contain custom JSON
};
...
struct NativeAssetImage {
    ~NativeAssetImage(){}
    ImageAssetTypes type;           /// < Type ID of the image element supported by the publisher
    Datacratic::TaggedInt w;                ///< width of ad in pixels
    Datacratic::TaggedInt wmin;     ///< The minimum requested width of the image in pixels
...
/** The Title object is to be used for title element of the Native ad. OpenRTB 2.3.
*/
...
/*****************************************************************************/
/* Native Asset Object OpenRTB 2.3                                           */
/*****************************************************************************/
...
struct NativeAsset{
    ~NativeAsset(){}
    Datacratic::TaggedInt id; ///<Unique asset ID, assigned by exchange.
    Datacratic::TaggedInt required; /// <Set to 1 if asset is required
    Datacratic::Optional<NativeAssetTitle> title; ///< Title object for title assets.
    Datacratic::Optional<NativeAssetImage> img; ///< The Image object to be used for all image elements of the Native ad such as Icons, Main Image,etc
    Datacratic::Optional<NativeAssetVideo> video; ///< Video object for video assets
    Datacratic::Optional<NativeAssetData> data;   ///<Data object for ratings, prices etc
};
...
struct NativeRequest {
    ...
    NativeLayoutID layout;              /// <The Layout ID of the native ad unit
    NativeAdUnitID adunit;              /// <The Ad unit ID of the native ad unit
    Datacratic::TaggedInt lcmtcnt;      /// < The number of identical placements in this Layout.
    Datacratic::TaggedInt seq;          ///<0 for the first ad, 1 for the second ad, and so on.
    std::vector<NativeAsset> assets;    ///< assets Native Asset Object
};
...
/** 3.2.5 Object: Native (OpenRTB 2.3)
    ...
    or Facebook post). As such, the response must be well-structured to afford the
    publisher fine-grained control over rendering */

struct Native {
    ~Native(){}
    Datacratic::Optional<NativeRequest> request; ///< NativeRequest
    std::string ver;                             ///< Application version, new in OpenRTB 2.3
    Datacratic::List<ApiFramework> api;          ///< Supported APIs (table 6.4)
    Datacratic::List<CreativeAttribute> battr;   ///< Blocked creative attributes (table 6.3)
    Json::Value ext;                             ///< Extensions go here, new in OpenRTB 2.3
};
...
struct Impression {
    ...
    Datacratic::Optional<Native> native;        ///Native object required if this impression is offered as a native ad opportunity.
...
struct Context {
    ...
    Datacratic::TaggedBool mobile;                 ///< new in OpenRTB 2.3. Mobile-optimized signal, where 0 = no, 1 = yes
...
struct Bid {
    ...
    std::vector<std::string> cat;             ///< new in OpenRTB 2.3. IAB content categories of the creative
