...
DefaultDescription()
{
    ...
    addField("native", &Impression::native, "This object represents a native type impression");
...
DefaultDescription()
{
     addField("len", &NativeAssetTitle::len, "Maximum length of the text in the title element");
     addField("ext", &NativeAssetTitle::ext, "This object is a placeholder that may contain custom JSON");
}

DefaultDescription<NativeAssetData>::
DefaultDescription()
{
    addField("type",  &NativeAssetData::type, "Type ID of the element supported by the publisher");
    addField("len",  &NativeAssetData::len, "Maximum length of the text in the element’s response");
}

DefaultDescription<NativeAssetVideo>::
DefaultDescription()
{
    addField("mimes",       &NativeAssetVideo::mimes, "Content MIME types supported");
    addField("minduration", &NativeAssetVideo::minduration, "Minimum video ad duration in seconds");
    addField("maxduration", &NativeAssetVideo::maxduration, "Maximum video ad duration in seconds");
    addField("protocols",   &NativeAssetVideo::protocols, "Bid response protocols array");
    addField("ext",         &NativeAssetVideo::ext, "This object is a placeholder that may contain custom JSON");
}

DefaultDescription<NativeAssetImage>::
DefaultDescription()
{
    addField("type",  &NativeAssetImage::type, "Type ID of the image element supported by the publisher");
    addField("w",     &NativeAssetImage::w, "Width of the image in pixels");
    addField("wmin",  &NativeAssetImage::wmin, "The minimum requested width of the image in pixels");
    addField("h",     &NativeAssetImage::h, "Height of the image in pixels");
    addField("hmin",  &NativeAssetImage::hmin, "The minimum requested height of the image in pixels");
}
...
DefaultDescription()
{
     addField("id",      &NativeAsset::id, "Unique asset ID, assigned by exchange");
     addField("required",&NativeAsset::required, "Set to 1 if asset is required");
     addField("title",   &NativeAsset::title, "Title object for title assets.");
     addField("img",     &NativeAsset::img, "The Image object to be used for all image elements of the Native ad such as Icons, Main Image,etc");
     addField("video",   &NativeAsset::video, "Video object for video assets");
     addField("data",    &NativeAsset::data, "Data object for ratings, prices etc");
}
...
DefaultDescription()
{
    addField("ver",     &NativeRequest::ver, "Extended impression attributes");
    ...
    addField("lcmtcnt", &NativeRequest::lcmtcnt, "The number of identical placements in this Layout");
    addField("seq",     &NativeRequest::seq, "0 for the first ad, 1 for the second ad, and so on");
    addField("assets",  &NativeRequest::assets, "Assets Native Asset Object");
}

DefaultDescription<Native>::
DefaultDescription()
{
    addField("request", &Native::request, "Represents a native type impression");
    addField("ver",     &Native::ver, "Application version");
    addField("api",     &Native::api, "Supported APIs");
    addField("battr",   &Native::battr, "Blocked creative attributes");
    addField("ext",     &Native::ext, "Extended impression attributes");
...
DefaultDescription()
{
    ...
    addField("mobile", &Context::privacypolicy, "Mobile-optimized signal, where 0 = no, 1 = yes");
