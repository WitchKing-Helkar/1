   ...
   getAdm( const Json::Value& reqAssets, const Json::Value& confAssets ) const
   {
      ...
      for ( auto & r : reqAssets )
      {
         ...
         if ( r.isMember( "data" ) )
         {
            ...
            for ( auto & data : assets.data[type] )
            {
               int len = r["data"]["len"].asInt();        
               if ( data.value.length() <= len || len == -1 )
               {
                  currAsset["data"]["value"] = data.value;
                  currAsset["id"] = r["id"].asInt();       
         ...
         if ( r.isMember( "img" ) )
         {
            ...
            if ( r["img"].isMember( "h" ) && r["img"].isMember( "w" ) )
            {
               int w = r["img"]["w"].asInt(),
                   h = r["img"]["h"].asInt();
               for ( auto & image : assets.image[type] )
               {
                  if ( image.h == h && image.w == w )
                  {
                     currAsset["img"]["url"] = image.link;
                     currAsset["img"]["w"]   = image.w;
                     currAsset["img"]["h"]   = image.h;
                     currAsset["id"] = r["id"].asInt();
            ...
            int wmin = r["img"]["wmin"].asInt(),
                hmin = r["img"]["hmin"].asInt();
            for ( auto & image : assets.image[type] )
            {
               if ( wmin <= image.w && hmin <= image.h )
               {
                      ...
                      ConfigAssets & assets ) const
    {
        for ( const Json::Value & val : json )
        {
            ...
            if ( val.isMember( "data" ) )
            {
                ...
                assets.data[type].push_back( ConfigAssets::DataAsset( val["data"]["value"].asString() ) );
            ...
            if ( val.isMember( "img" ) )
            {
                ...
                assets.image[type].push_back( ConfigAssets::ImageAsset( val["img"]["w"].asInt(), val["img"]["h"].asInt(), val["img"]["url"].asString() ) );
