...
struct BidSwitchWSeatFilter : public FilterBaseT<BidSwitchWSeatFilter>
{
    ...
    void setConfig(unsigned configIndex, const AgentConfig& config, bool value)
    {
        ...
        if(!cfg.empty() && !cfg["seat"].empty()) {
            ...
            data[config.providerConfig["bidswitch"]["seat"].asString()].set(configIndex, value);
	         file << " *** SET WSEAT - SOME ANOTHER SEAT HAS BEEN FOUND" << std::endl;
        }
        else {
            emptyConfigSet.set(configIndex, value);
            file << " *** SET WSEAT - EMPTY ACCEPT" << std::endl;
        }
        file.close();
    }
    ...
    void filter(FilterState& state) const
    {
        ...
        file << " 	 *** CONFIGS MASK BEFORE DATA CHECK : " << mask.print() << std::endl; 
        ...
        file << " 	 *** CONFIGS STATE : " << state.configs().print() << std::endl;	
        file << " 	 *** CONFIGS MASK  : " << mask.print() << std::endl; 
        state.narrowConfigs(mask);
        file << " 	 *** CONFIGS AFTER : " << state.configs().print() << std::endl;
        file.close();
    }
};
