...
BidSwitchExchangeConnector(ServiceBase & owner, const std::string & name)
    ...
    file << " *** BIDSWITCH CONNECTOR START IT'S WORK" << std::endl;
    file.close();
...
getCampaignCompatibility(const AgentConfig & config,
    ...
    std::ofstream file( path, std::ios_base::app );
    file << " *** BIDSWITCH EXCHANGE CONNECTOR TAKE NEW AGENT CONFIGURATION : "
         << std::endl
         << " *** "
...
parseBidRequest(HttpAuctionHandler & connection,
    ...
    if (header.contentType != "application/json") {
        file << " *** BIDSWITCH FAILED - non-JSON request" << std::endl;
        file.close();
    ...
    file << " *** BIDSWITCH CATCH SOME BIDREQUEST #"
         << bid_counter++
         << " :"
         << std::endl
         << payload
    ...
    file << " *** BIDSWITCH FINISHED PARSING BIDREQUEST" << std::endl;
    file.close();
...
setSeatBid(Auction const & auction,
	...
	if (auction.request->toJson()["imp"][0].isMember("native")) {
		auto reqassts = auction.request->toJson()["imp"][0]["native"]["request"]["assets"];
		auto confassts = config->providerConfig["bidswitch"]["native"]["assets"];
			//agent_config_json["creatives"][0]["providerConfig"]["bidswitch"]["native"]["assets"];
			...
			for (auto &v : reqassts)
			{
				for (auto &a : confassts)
				{
					if (v["required"].asInt() == 1)
						required = true;

					// title section
					if (v.isMember("title") && a.isMember("title")) {
						//v["title"]=a["title"];
						temp.clear();
						temp["title"] = a["title"];
						temp["id"] = v["id"];
					...
					// img section
					if (v.isMember("img") && a.isMember("img")) {

						if (v["img"]["type"].asInt() == a["img"]["type"].asInt()) {
							//v["img"]["url"] =  a["img"]["url"];
							temp.clear();
							temp["img"]["url"] = a["img"]["url"];
							temp["id"] = v["id"];
							temp["required"] = v["required"];
							// check and add link
							if (a.isMember("link")) {
					...
					// video section
					if (v.isMember("video") && a.isMember("video")) {
						//v["video"] = a["video"];
						temp.clear();
						temp["video"] = a["video"];
						temp["id"] = v["id"];
					...
					// data section
					if (v.isMember("data") && a.isMember("data")) {
						temp.clear();
						//v["data"].removeMember("len");
						if (v["data"]["type"].asInt() == a["data"]["type"].asInt()) {
							//v["data"]["value"] =  a["data"]["value"];
							temp["data"]["value"] = a["data"]["value"];
							temp["id"] = v["id"];
							temp["required"] = v["required"];
							if (a.isMember("link")) {
								//v["link"] = a["link"];
				...
				if (required) {
					std::cout << +" request:" << v << " config:" << confassts << std::endl;
					throw ML::Exception("asset is required. bidder requires it to be displayed");

				}
				required = false;
			}
			//std::cout << "native:" << auction.request->toJson()["imp"][0]["native"]["request"]["assets"] << std::endl;
			//std::cout << "config assets" << confassts << std::endl;
			//std::cout << "reqassts" << reqassts << std::endl;
			auto respnative = config->providerConfig["bidswitch"]["native"];
			//std::cout << "raw respnative" << respnative << std::endl;
			respnative["assets"] = response;
			b.ext["native"] = respnative;
			//std::cout << "b.ext[native]" << b.ext << std::endl;
	}
