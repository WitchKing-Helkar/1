...
createBidRequestHelper(OpenRTB::BidRequest & br,
	...
	/*
	if (ctx.br->imp[0].native)
		std::cout << "createBidRequestHelper : br " << ctx.br->toJson() << std::endl;
	*/
...
onImpression(OpenRTB::Impression & impression) {
    // std::cout << "debug -> onImpression 2.0" << std::endl;
    //
...
onBidRequest(OpenRTB::BidRequest & br) {
    //std::cout << "debug -> onBidRequest"  << std::endl;
    // Call V1
    OpenRTBBidRequestParser::onBidRequest(br);
	...
	//std::cout << "debug ->onBidRequest -> end " << ctx.br->toJson() << std::endl;
}
...
onImpression(OpenRTB::Impression & imp) {
    ...
    // Deal with secure, business logic
    //std::cout << "debug -> onImpression -> onImpression 2.3" << std::endl;
    //


    if(imp.native) {
        this->onNative(*imp.native);
    }
...
onDevice(OpenRTB::Device & device) {
	...
	ctx.br->segments.add("devicetype", device.devicetype.val);

    if(device.devicetype.val > 7)
        LOG(OpenRTBBidRequestLogs::error22) << "Device Type : " << device.devicetype.val << " not supported in OpenRTB 2.3." << endl;

	ctx.br->segments.add("os", device.os.rawString());

	ctx.br->segments.add("osv", device.osv.rawString());

	ctx.br->segments.add("model", device.model.rawString());
...
void
OpenRTBBidRequestParser2point3::
onNative(OpenRTB::Native & ntv) {

	//std::cout << "debug ->onNative -> onImpression 2.3" << std::endl;

    if(ntv.request) {
        this->onNativeRequest(*ntv.request);
    }
...
void
OpenRTBBidRequestParser2point3::
onNativeRequest(OpenRTB::NativeRequest & ntvRequest) {

	//std::cout << "debug ->onNativeRequest -> onImpression 2.3" << std::endl;
	//std::cout << "debug ->onNativeRequest -> begin: " << ctx.br->toJson() << std::endl;

	//std::cout << "debug ->onNativeRequest -> ver" << ntvRequest.ver.value() << std::endl;
	ctx.br->segments.add("ver", ntvRequest.ver.value());
	

    // Put them into a segment called layout which we can use for filtering after
	ctx.br->segments.add("layout", ntvRequest.layout.value());

	ctx.br->segments.add("adunit", ntvRequest.adunit.value());

	//std::cout << "debug ->onNativeRequest ->end " << ctx.br->toJson() << std::endl;
}
