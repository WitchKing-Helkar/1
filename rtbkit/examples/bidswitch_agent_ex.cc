/** bidding_agent_ex.cc                                 -*- C++ -*-
    Sviatoslav Borysiuk, 1 Sep 2015
    Copyright (c) 2013 Datacratic.  All rights reserved.

    Example of a simple fixed-price bidding agent that confgurated for bidswitch exchange.
...
/** Simple bidding agent for bidswitch exhange
 */
 
struct BidSwitchBiddingAgent :
        public BiddingAgent
{
    std::string configFile;
    std::string logFile;
    
    BidSwitchBiddingAgent(
            std::shared_ptr<Datacratic::ServiceProxies> services,
            const string& serviceName) :
        BiddingAgent(services, serviceName),
        accountSetup(false)
    {
        configFile = "/home/dev/rtbkit/rtbkit/examples/bidswitch_agent_configuration.json";
        logFile    = "/home/dev/rtbkit/rtbkit_log.txt";
    }
    ...
    void init(const std::shared_ptr<ApplicationLayer> appLayer)
    {
        ...
        onBidRequest = bind(
                &BidSwitchBiddingAgent::bid, this, _1, _2, _3, _4, _5, _6);
        ...
        addPeriodic("BidSwitchBiddingAgent::pace", 10.0,
                [&] (uint64_t) { this->pace(); });
    ...
    */
            ...
            const Json::Value & augmentations)
    {
        ...
        for (Bid& bid : bids) {
            ...
            if ( rand() / 2 ) { price = 500 + rand() % 300; } else { price = 500 - rand() % 300 ; }
...
int main(int argc, char** argv)
{
    ...
    RTBKIT::BidSwitchBiddingAgent agent(serviceProxies, "bidswitch-agent-ex");
