...
struct NativeCompareFilter : public FilterBaseT<NativeCompareFilter>
{
   ...
   struct ConfigAssets
   {
      ...
      struct ImageObject
      {
         ImageObject( const int & _w = -1, const int & _h = -1, const std::string & _mimeType = "" ) :
            w( _w ),
            h( _h ),
            mimeType( _mimeType )
         {}
      ...
      std::unordered_map<int, std::vector<DataObject>>  data;
      std::unordered_map<int, std::vector<ImageObject>> image;
      TitleObject title;
      VideoObject video;
   ...
   typedef std::multimap<int, std::shared_ptr<ConfigAssets>> ConfigsMapType;
      ...
      const std::shared_ptr<ConfigAssets> & configAssets ) const
   {
      ...
      {
         if ( asset.required.value() != 1 )    // skip it, if asset isn't nessecary for response
         {
            continue;
         }
         if ( asset.title )   // title-asset
         {
            ...
            file << "         *** successfully check title asset"
                 << std::endl;
            continue;
         }
         ...
         if ( asset.img )     // image-asset
         {
            ...
            {
               file << "         *** request has fixed height and width for asset image(type #" 
                    << std::to_string( type ) 
                    << ")"
                    << std::endl;
               bool success = false;
               int w = asset.img->w.value(),
                   h = asset.img->h.value();
               for ( auto & image : configAssets->image[type] )
               {
                  if ( image.w != w || image.h != h )
                  {         
                     file << "         *** current example of image(type #" 
                          << std::to_string( type ) 
                          << ") field doesn't satisfy request by it's format : "
                          << std::endl
                          << "             "
                          << std::to_string( w )
                          << "x"
               ...
               if ( !success )
               {    
                  file << "         *** request has not passed by image(type #" << type << ") format sizes"
                       << std::endl;
                  file.close();
               ...
               file << "         *** successfully check data(type #"
                    << std::to_string( type )
                    << ") asset"
                    << std::endl;
               continue;            
            ...
            {
               file << "         *** request has only minimal height and width for asset image(type #" 
                    << std::to_string( type ) 
                    << ")"
                    << std::endl;
               bool success = false;
               int wmin = asset.img->wmin.value(),
                   hmin = asset.img->hmin.value();
               for ( auto & image : configAssets->image[type] )
               {
                  if ( image.w < wmin || image.h < hmin )
                  {
                     file << "         *** current example of image(type #" 
                          << std::to_string( type ) 
                          << ") field doesn't satisfy request by it's minimal format sizes"
                          << std::endl;
                  }
                  else
                  {
                     float requestRatio = wmin / hmin,
                           configRatio  = image.w / image.h;
                     if ( ( 0.8 * requestRatio ) > configRatio || ( 1.2 * requestRatio ) < configRatio )
                     {
                        file << "         *** current example of image(type #" 
                          << std::to_string( type ) 
                          << ") field doesn't satisfy request by it's ratio to minimal format sizes"
                          << std::endl;
                     }      
                     else
                     {
               ...
               if ( !success )
               {
                  file << "         *** request has not passed by image(type #" << type << ") minimal format sizes"
                       << std::endl;
                  file.close();
                  return 0;
               }             
         ...
         if ( asset.data )    // data-asset
         {
            ...
            bool success = false;
            for ( auto & data : configAssets->data[type] )
            {
               if ( asset.data->len.value() != -1 )
               {
                  if ( asset.data->len.value() < data.len )
                  {
                     file << "         *** current example of data(type #" 
                          << std::to_string( type ) 
                          << ") field doesn't satisfy request by it's length : "
                          << std::endl
                          << "         *** "
      ...
      std::shared_ptr<ConfigAssets> & configAssets) const
   {
      for ( auto asset : nativeAssets )
      {
         ...
         if ( asset.isMember( "data" ) )
         {
            ...
            configAssets->data[type].push_back( ConfigAssets::DataObject( asset["data"]["value"].asString().length() ) );
         }
         if ( asset.isMember( "img" ) )
         {
            ...
            {
               configAssets->image[type].push_back( ConfigAssets::ImageObject( asset["img"]["w"].asInt(), asset["img"]["h"].asInt() ) );
            }
         }
