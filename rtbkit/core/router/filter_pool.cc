...
filter(const BidRequest& br, const ExchangeConnector* conn, const ConfigSet& mask)
{
    ...
    file << " *** REQUEST : " << std::endl << state.request.toJson().toString() << std::endl;

    for (FilterBase* filter : current->filters) {
        file << "     *** NOW WE RUN FILTER : " << filter->name() << std::endl;
        filter->filter(state);
        ...
        if (filtered.empty()) {
            file << "     *** FILTER " << filter->name() << " BREAK THE LOOP" << std::endl;
            if (sampleStats) 
