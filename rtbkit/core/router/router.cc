...
start(boost::function<void ()> onStop)
{
    ...
    file << " *** ROUTER START IT'S WORK" << std::endl;
    file.close();
                         ...
                         bool includeReasons)
{
    ...
    file << " *** ROUTER CONFIGURATE AGENT " << agent << " ON " << name << " EXCHANGE BEGIN *** " << std::endl;

    auto ecomp = exchange->getCampaignCompatibility(config, includeReasons);
    if(!ecomp.isCompatible) {
        file << "     *** CAMPAIGN NOT COMPATIBLE FOR \""<< name << " \" EXCHANGE AND AGENT \"" << agent << "\" : " << ecomp.reasons << std::endl;
        file << " *** ROUTER.CC CONFIGURATE AGENT ON " << name << " EXCHANGE END *** " << std::endl;
        file.close();
    ...
    for(auto & c : config.creatives) {
        ...
        if(!ccomp.isCompatible) {
            file << "     *** FAILED CONFIGURATE AGENT " << agent << " ON " << name << " EXCHANGE : " << ccomp.reasons << std::endl;
            ...
            file << "     *** SUCCESSFULLY CONFIGURATED AGENT " << agent << " ON " << name << " EXCHANGE" << std::endl;
        }
    }

    if (numCompatibleCreatives == 0) {
        cerr << "no compatible creatives" << endl;
        file << "     NO COMPATIBLE CREATIVES" << std::endl;
        this->recordHit("%s.noCompatibleCreative", name);
        file << " *** ROUTER.CC CONFIGURATE AGENT " << agent << " ON " << name << " EXCHANGE END *** " << std::endl;
        file.close();
        return;
    }

    std::lock_guard<ML::Spinlock> guard(config.lock);
    file << "     *** NAME \"" << name << "\" AGENT \"" << config.account.toString() << "\" ***" << std::endl;
