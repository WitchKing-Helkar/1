...
createFromJson(const Json::Value & json)
{
    ...
    for (auto it = json.begin(), end = json.end(); it != end;  ++it) {
        ...
        else if (it.memberName() == "nativeLayoutFilter")
            newConfig.nativeLayoutFilter.fromJson(*it, "nativeLayoutFilter");
        else if (it.memberName() == "nativePreFilter")
            newConfig.nativePreFilter = json.atStr( it.memberName() ).asInt();
...
toJson(bool includeCreatives) const
{
    ...
    if (!nativeLayoutFilter.empty())
        result["nativeLayoutFilter"] = nativeLayoutFilter.toJson();
    if (nativePreFilter != -1)
        result["nativePreFilter"] = nativePreFilter;
